...
export default function Sidebar() {
  ...
  const handleNavigation = (path) => {
    const isOnGamePage = currentPath.includes('/games/');
  ...
  // Categories matching what's visible in the screenshot
  const categories = [
    { id: 'home', name: 'Accueil', icon: 'home', path: '/' },
    { id: 'popular', name: 'Populaires', icon: 'fire', path: '/popular' },
    { id: 'new', name: 'Nouveautés', icon: 'sparkles', path: '/new' },
    { id: 'action', name: 'Action', icon: 'lightning-bolt', path: '/action' },
    { id: 'adventure', name: 'Aventure', icon: 'globe', path: '/adventure' },
    { id: 'puzzle', name: 'Puzzle', icon: 'puzzle', path: '/puzzle' },
    { id: 'sports', name: 'Sports', icon: 'trophy', path: '/sports' },
    { id: 'shooting', name: 'Tir', icon: 'target', path: '/shooting' },
    { id: 'racing', name: 'Course', icon: 'flag', path: '/racing' },
    { id: 'strategy', name: 'Stratégie', icon: 'chess-king', path: '/strategy' },
    { id: 'simulation', name: 'Simulation', icon: 'desktop-computer', path: '/simulation' },
    { id: 'clicker', name: 'Clicker', icon: 'cursor-click', path: '/clicker' },
    { id: 'io', name: '.io', icon: 'globe-alt', path: '/io' },
    { id: 'trending', name: 'Tendance', icon: 'fire', path: '/trending' },
    { id: 'embedded', name: 'Embedded', icon: 'code', path: '/embedded' }
  ];
  ...
  return (
    <aside className="fixed top-0 left-0 h-screen w-20 bg-gray-900 border-r border-gray-700/50 flex flex-col items-center py-4 shadow-md z-30">
      {/* Logo */}
      ...
      {/* Navigation */}
      <nav className="flex-1 w-full flex flex-col items-center space-y-1 overflow-y-auto scrollbar-hide px-2">
        {categories.map(category => {
          ...
          return (
            <button
              ...
              className={`
                relative p-2 w-full flex flex-col items-center justify-center rounded-md 
            ...
            >
              {/* Active indicator */}
              {isActive && (
                <span className="absolute left-0 top-1/2 transform -translate-y-1/2 h-6 w-1 bg-indigo-500 rounded-r-full"></span>
              )}
              {renderIcon(category.icon)}
              <span className="text-[10px] mt-1 font-medium text-center leading-tight">{category.name}</span>
            </button>
          );
        })}
      </nav>
...
Sidebar.propTypes = {
  // Add any props if Sidebar receives them, otherwise leave empty or remove.
}; 